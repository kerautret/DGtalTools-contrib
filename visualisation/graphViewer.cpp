...
{ 
  ...
  if ( colormapOpt->count() > 0 )
  {
    for ( const auto& e: vectEdges )
    {
      ...
      radii =   { cstRadSectionEdge ? std::min(vectRadii[e[0]], vectRadii[e[1]]): vectRadii[e[0]],
                  cstRadSectionEdge ?  std::min(vectRadii[e[0]], vectRadii[e[1]]) : vectRadii[e[1]] };
